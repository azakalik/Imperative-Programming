...
//  socialADT.c
//  Parcial2_2020_01
//  En esta versión se validan todos los alloc. Es responsabilidad del front verificar errno
//
...
// En caso de error setea errno en ENOMEM
...
static TPerson addPersonRec(TPerson first, const char * name, int *ok){
    ...
    if ( first == NULL || ( c = strcmp(first->name, name)) > 0) {
        ...
        if ( aux==NULL || errno == ENOMEM) {
           return first;  // ante un error retorna la misma lista, sin hacer cambios
        }
        aux->firstR = NULL;
        aux->sizeR = 0;
...
static void freeVector(char * vec[], size_t dim) {
    for(int i=0; i<dim; i++) {
        free(vec[i]);
    }
    free(vec);
...
char ** related(const socialADT soc, const char * person) {
    ...
    if ( p != NULL) {
        ...
        while ( auxR != NULL) {
            ...
            if ( ans[k]==NULL) {
                // se produjo un error, liberar todo lo reservado
                freeVector(ans, k);
                return NULL;
            }
            k++;
